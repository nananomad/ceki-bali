...
function aiDecide(player, topDiscard) {
  ...
  // ================================================================
  // PRINSIP DASAR:
  // Mengambil dari Discard → kartu masuk Open Pile (TIDAK BISA DIBUANG)
  // → Mengurangi fleksibilitas hand secara permanen
  // → Harus bijak: manfaat harus JELAS, biaya harus TERKENDALI
  // ================================================================

  const handCards = [...player.hand];
  const handCount = handCards.filter(c => c.id === topDiscard.id).length;
  const openCount  = player.openPile.filter(c => c.id === topDiscard.id).length;
  const sameIdSupport    = countSameId(handCards, topDiscard);
  const sameValueSupport = countSameValueNonHonours(handCards, topDiscard);

  // ── STEP 1: GANTUNG SAFETY CHECK ──
  // Sebelum apapun, hitung berapa kartu di Hand yang bisa dibuang
  // setelah take (HANYA dari Hand, bukan OpenPile).
  // Kartu yang boleh dibuang: tidak GANTUNG, bukan bagian Soca/Patuh inti.
  function discardHeadroom() {
    const currentResult = findBestCombo([...handCards, ...player.openPile]);

    // Kartu yang merupakan inti Soca (3 identik) — jangan dikorbankan
    const socaUids = new Set();
    for (const combo of currentResult.combos) {
      if (combo.type === 'soca') combo.cards.forEach(c => socaUids.add(c.uid));
    }

    // Hitung kartu Hand yang bisa dibuang tanpa mengorbankan Soca
    let safe = 0;
    for (const c of handCards) {
      if (socaUids.has(c.uid)) continue;          // bagian Soca → jangan buang
      if (!canDiscard(c, player.id, false)) continue; // GANTUNG
      safe++;
    }
    return safe;
  }

  const headroom = discardHeadroom();

  // Jika kurang dari 2 pilihan buang yang aman → JANGAN AMBIL apapun
  // (mengambil akan membuat hand semakin terkunci)
  if (headroom < 2) {
    return { takeDiscard:false, reason:'insufficient-headroom', headroom };
  }

  // ── STEP 2: SOCA FORMATION — Prioritas Tertinggi ──

  // Sudah punya Soca lengkap via openPile → skip
  const alreadyHasSocaWithOpen = player.openPile.some(op =>
    op.id === topDiscard.id &&
    handCards.filter(c => c.id === op.id).length >= 2
  );
  if (alreadyHasSocaWithOpen) {
    return { takeDiscard:false, reason:'soca-already-complete' };
  }

  // Hand x2 + topDiscard → Soca Formation (ambil → openPile → lock)
  // Ini SELALU worth it: manfaat maksimal (Soca)
  if (handCount >= 2) {
    return { takeDiscard:true, reason:'soca-formation-trigger' };
  }

  // Hand x1 + OpenPile x1 + topDiscard → Soca Formation
  if (handCount >= 1 && openCount >= 1) {
    return { takeDiscard:true, reason:'soca-formation-three-source' };
  }

  // ── STEP 3: SRIGAT → PHASE TRIGGER ──
  // Strategi tinggi: reorganisasi hand → trigger MECARI/MEJAGA
  if (wouldSrigatTriggerPhase(player, topDiscard)) {
    return { takeDiscard:true, reason:'srigat-triggers-phase' };
  }

  // ── STEP 4: PHASE ENTRY LANGSUNG ──
  // Jika mengambil topDiscard + buang 1 kartu → langsung MECARI/MEJAGA
  for (const discardCandidate of handCards) {
    if (!canDiscard(discardCandidate, player.id, false)) continue;
    const simHand = handCards.filter(c => c.uid !== discardCandidate.uid);
    const simAll  = [...simHand, ...player.openPile, topDiscard];
    if (simAll.length === 11 && wouldEnterPhase(simAll)) {
      // Pastikan headroom masih cukup setelah take
      if (headroom >= 2) {
        return { takeDiscard:true, reason:'phase-entry-direct' };
      }
    }
  }

  // ── STEP 5: MANFAAT JELAS — Patuh Dua Murni ──
  // topDiscard identik dengan TEPAT 1 kartu di Hand
  // → Membentuk pasangan identik (kandidat Patuh Dua / Soca berikutnya)
  // Syarat: headroom masih >= 3 (ambil mengurangi fleksibilitas)
  if (sameIdSupport === 1 && headroom >= 3) {
    // Bonus: jika juga punya 1 angka sama → bisa jadi Patuh Dua lengkap
    if (sameValueSupport >= 1) {
      return { takeDiscard:true, reason:'patuh-dua-candidate-strong' };
    }
    // Patuh Dua murni (hanya 1 identik) → ambil HANYA jika headroom aman
    if (headroom >= 4) {
      return { takeDiscard:true, reason:'patuh-dua-candidate' };
    }
  }

  // ── STEP 6: SRIGAT MURNI ──
  // topDiscard angka sama dengan >= 2 kartu di Hand (beda jenis)
  // → Membentuk Srigat (3 angka sama, 3 jenis berbeda)
  // Manusia ambil ini jika kombinasi sudah kuat
  if (sameValueSupport >= 2 && headroom >= 3) {
    // Pastikan ketiga kartu beda jenis (syarat Srigat)
    const sameValInHand = handCards.filter(c =>
      c.type !== 'honours' &&
      c.value === topDiscard.value &&
      c.type !== topDiscard.type
    );
    const allTypes = new Set([topDiscard.type, ...sameValInHand.map(c => c.type)]);
    if (allTypes.size >= 3) {
      return { takeDiscard:true, reason:'srigat-candidate' };
    }
  }

  // ── STEP 7: Honours khusus ──
  // Honours hanya diambil jika sudah punya 2+ Honours di hand
  // (untuk Soca Honours atau Patuh Honours)
  if (topDiscard.type === 'honours') {
    const honoursInHand = handCards.filter(c => c.type === 'honours').length;
    if (honoursInHand >= 2 && headroom >= 3) {
      return { takeDiscard:true, reason:'honours-set-complete' };
    }
    // Kurang dari 2 Honours → honours terlalu spekulatif, skip
    return { takeDiscard:false, reason:'honours-insufficient-support' };
  }

  // ── DEFAULT: Jangan ambil ──
  // Jika tidak ada manfaat yang jelas dan terukur → draw dari Deck
  // Lebih baik dapat kartu random dari Deck daripada memblokir hand
  return { takeDiscard:false, reason:'draw-preferred' };
}
